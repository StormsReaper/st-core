...
local dmvPeds = {}
local dmvBlips = {}
function STDMV.Open() SetNuiFocus(true, true); SendNUIMessage({ action = 'open' }); TriggerServerEvent('st-core:server:dmvOpen') end
function STDMV.OpenInsuranceClaim() SetNuiFocus(true, true); SendNUIMessage({ action = 'openClaim' }); TriggerServerEvent('st-core:server:claims:getVehicles') end
...
local function createDMVBlips()
    ...
    for _, location in ipairs(Config.DMV.Locations or {}) do
        local blip = AddBlipForCoord(location.x, location.y, location.z); SetBlipSprite(blip, Config.DMV.Blip.Sprite or 498); SetBlipDisplay(blip, 4); SetBlipScale(blip, Config.DMV.Blip.Scale or 0.8); SetBlipColour(blip, Config.DMV.Blip.Color or 3); SetBlipAsShortRange(blip, true)
        ...
        for index, location in ipairs(Config.DMV.Locations or {}) do
            ...
            if distance < 15.0 then
                ...
                if ped and DoesEntityExist(ped) and distance < interactionDistance then
                    BeginTextCommandDisplayHelp('STRING'); AddTextComponentSubstringPlayerName('Press ~INPUT_CONTEXT~ to speak with the DMV clerk'); EndTextCommandDisplayHelp(0, false, true, -1); if IsControlJustReleased(0, 38) then STDMV.Open() end
                end
            end
        end; Wait(wait)
    end
...
RegisterNUICallback('submitInsuranceClaim', function(data, cb) TriggerServerEvent('st-core:server:claims:submit', data); cb({ ok = true }) end)
