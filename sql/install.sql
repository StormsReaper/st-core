-- st-core vehicle registration, insurance, DMV and purchase schema
-- Requires MySQL/MariaDB and oxmysql.
...
CREATE TABLE IF NOT EXISTS st_vehicle_registrations (
    ...
    PRIMARY KEY (id), UNIQUE KEY uq_vehicle_registration_vehicle (vehicle_identifier),
    UNIQUE KEY uq_vehicle_registration_plate (plate), KEY idx_vehicle_registration_owner (owner_identifier),
...
CREATE TABLE IF NOT EXISTS st_vehicle_purchases (
    ...
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (id), UNIQUE KEY uq_vehicle_purchase_identifier (vehicle_identifier),
...
CREATE TABLE IF NOT EXISTS st_insurance_plans (
    ...
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (id), KEY idx_insurance_plans_active (active)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS st_vehicle_insurance (
    ...
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (id), UNIQUE KEY uq_vehicle_insurance_policy (policy_number),
    KEY idx_vehicle_insurance_vehicle (vehicle_identifier), KEY idx_vehicle_insurance_owner (owner_identifier),
...
CREATE TABLE IF NOT EXISTS st_insurance_claims (
    ...
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (id), KEY idx_insurance_claims_policy (policy_id), KEY idx_insurance_claims_claimant (claimant_identifier),
...
ON DUPLICATE KEY UPDATE name=VALUES(name), description=VALUES(description), coverage_type=VALUES(coverage_type),
monthly_premium=VALUES(monthly_premium), liability_limit=VALUES(liability_limit), collision_limit=VALUES(collision_limit),
comprehensive_limit=VALUES(comprehensive_limit), deductible=VALUES(deductible), updated_at=CURRENT_TIMESTAMP;
