...
local function generateVIN()
    for _ = 1, 50 do
        ...
        if not MySQL.single.await('SELECT id FROM st_vehicle_registrations WHERE vin = ? LIMIT 1', { vin })
            and not MySQL.single.await('SELECT id FROM st_vehicle_records WHERE vin = ? LIMIT 1', { vin }) then
            return vin
        end
    end
    return nil
...
local function getFrameworkVehicle(ownerIdentifier, plate)
    ...
    if framework == 'qbcore' then
        return MySQL.single.await('SELECT * FROM player_vehicles WHERE citizenid = ? AND plate = ? LIMIT 1', { ownerIdentifier, plate })
    elseif framework == 'esx' then
        return MySQL.single.await('SELECT * FROM owned_vehicles WHERE owner = ? AND plate = ? LIMIT 1', { ownerIdentifier, plate })
    end
    return nil
end
...
-- Looks up the authoritative framework vehicle record by the plate entered at the DMV.
-- This does not require a JG purchase callback and therefore works with JG, other dealerships,
-- admin-gifted vehicles, imports, or vehicles inserted directly into the framework table.
function STVehicles.LookupOwnedVehicleForDMV(ownerIdentifier, plate)
    plate = STValidation.NormalizePlate(plate)
    if plate == '' or #plate > 12 then return nil, 'invalid_plate' end
    ...
    if existing then
        return { alreadyRegistered = true, registration = existing, framework = record, plate = plate }
    end
    ...
    return {
        ...
        ownerName = STPayments.GetName(source) or 'Unknown',
        purchasePrice = tonumber(record.price or record.purchase_price) or 0,
        purchaseType = record.payment or record.paymentMethod,
        paymentMethod = record.payment or record.paymentMethod,
        financed = record.finance == 1 or record.finance == true or record.financed == 1 or record.financed == true,
        framework = record
    }
end
...
function STVehicles.UpdateOwnedVehiclePlate(ownerIdentifier, oldPlate, newPlate)
    ...
    oldPlate = STValidation.NormalizePlate(oldPlate); newPlate = STValidation.NormalizePlate(newPlate)
...
function STVehicles.UpdateFrameworkOwner(oldOwnerIdentifier, newOwnerIdentifier, plate)
    local framework = getFramework(); local affected
    if framework == 'qbcore' then affected = MySQL.update.await('UPDATE player_vehicles SET citizenid = ? WHERE citizenid = ? AND plate = ?', { newOwnerIdentifier, oldOwnerIdentifier, plate })
    elseif framework == 'esx' then affected = MySQL.update.await('UPDATE owned_vehicles SET owner = ? WHERE owner = ? AND plate = ?', { newOwnerIdentifier, oldOwnerIdentifier, plate })
    else return false, 'unsupported_framework' end
...
function STVehicles.RegisterVehicle(data)
    ...
    if isCustom then
        ...
        plate = generateUniquePlate(); if not plate then return false, 'plate_generation_failed' end
    end
    local vin = data.vin or generateVIN(); if not vin then return false, 'vin_generation_failed' end
    local now = os.time(); local expiresAt = now + ((Config.Registration.DurationDays or 30) * 86400)
    local insertId = MySQL.insert.await([[INSERT INTO st_vehicle_registrations
        (vehicle_identifier, vin, owner_identifier, owner_name, vehicle_model, vehicle_display_name,
         purchase_price, purchase_type, payment_method, financed, dealership, plate, plate_type,
         registered_at, expires_at, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'active')]], {
        data.vehicleIdentifier, vin, data.ownerIdentifier, data.ownerName or 'Unknown', data.vehicleModel,
        data.vehicleDisplayName, tonumber(data.purchasePrice) or 0, data.purchaseType, data.paymentMethod,
        data.financed and 1 or 0, data.dealership, plate, isCustom and 'custom' or 'standard', now, expiresAt
    })
...
function STVehicles.SyncVehicleRecord(vehicleIdentifier)
    ...
    MySQL.query.await([[INSERT INTO st_vehicle_records
        (vehicle_identifier,vin,plate,owner_identifier,owner_name,vehicle_model,vehicle_display_name,purchase_price,dealership,
         registration_status,registration_expires_at,insurance_policy_number,insurance_status,insurance_expires_at)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE vin=VALUES(vin),plate=VALUES(plate),owner_identifier=VALUES(owner_identifier),
        owner_name=VALUES(owner_name),vehicle_model=VALUES(vehicle_model),vehicle_display_name=VALUES(vehicle_display_name),purchase_price=VALUES(purchase_price),
        ...
        insurance_policy_number=VALUES(insurance_policy_number),insurance_status=VALUES(insurance_status),insurance_expires_at=VALUES(insurance_expires_at),updated_at=CURRENT_TIMESTAMP]], {
        registration.vehicle_identifier,registration.vin,registration.plate,registration.owner_identifier,registration.owner_name or 'Unknown',registration.vehicle_model,
        registration.vehicle_display_name,registration.purchase_price or 0,registration.dealership,registration.status,registration.expires_at,insurance and insurance.policy_number or nil,
        insurance and insurance.status or nil,insurance and insurance.expires_at or nil
    })
    return true
end
...
function STVehicles.IsRegistered(plate)
    local registration=STVehicles.GetRegistrationByPlate(plate);if not registration or registration.status~='active' then return false end
    return tonumber(registration.expires_at)==nil or tonumber(registration.expires_at)>=os.time()
end

exports('GenerateVehiclePlate',STVehicles.GeneratePlate);exports('GenerateVehicleVIN',STVehicles.GenerateVIN);exports('ValidateVehiclePlate',STVehicles.ValidatePlate)
exports('GetVehicleRegistration',STVehicles.GetRegistration);exports('GetVehicleRegistrationByPlate',STVehicles.GetRegistrationByPlate);exports('GetVehicleRecordByPlate',STVehicles.GetVehicleRecordByPlate)
exports('GetVehicleRecordByVIN',STVehicles.GetVehicleRecordByVIN);exports('RegisterVehicle',STVehicles.RegisterVehicle);exports('RenewVehicleRegistration',STVehicles.RenewRegistration)
exports('IsVehicleRegistered',STVehicles.IsRegistered);exports('GetOwnedVehicleByPlate',STVehicles.GetOwnedVehicleByPlate);exports('UpdateOwnedVehiclePlate',STVehicles.UpdateOwnedVehiclePlate)
exports('UpdateFrameworkVehicleOwner',STVehicles.UpdateFrameworkOwner);exports('SyncVehicleRecord',STVehicles.SyncVehicleRecord);exports('LookupOwnedVehicleForDMV',STVehicles.LookupOwnedVehicleForDMV)
