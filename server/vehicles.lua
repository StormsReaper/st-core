...
local function plateExists(plate)
    if registrationPlateExists(plate) then
        return true
    end
...
local function generateVIN()
    for _ = 1, 50 do
        ...
        local registrationExists = MySQL.single.await('SELECT id FROM st_vehicle_registrations WHERE vin = ? LIMIT 1', { vin })
        local recordExists = MySQL.single.await('SELECT id FROM st_vehicle_records WHERE vin = ? LIMIT 1', { vin })
        if not registrationExists and not recordExists then
            return vin
        end
    end
    return nil
...
function STVehicles.GeneratePlate()
    return generateUniquePlate()
end

function STVehicles.GenerateVIN()
    return generateVIN()
end

function STVehicles.ValidatePlate(plate, allowCustom)
    return STValidation.IsValidPlate(plate, allowCustom)
end

function STVehicles.GetRegistrationByPlate(plate)
    plate = STValidation.NormalizePlate(plate)
    if not STValidation.IsValidPlate(plate, true) then
        return nil
    end
...
function STVehicles.GetVehicleRecordByVIN(vin)
    ...
    vin = vin:upper()
    return MySQL.single.await('SELECT * FROM st_vehicle_records WHERE vin = ? LIMIT 1', { vin })
end

function STVehicles.GetOwnedVehicleByPlate(ownerIdentifier, plate)
    if not STValidation.IsIdentifier(ownerIdentifier) then
        return nil
    end
...
function STVehicles.UpdateOwnedVehiclePlate(ownerIdentifier, oldPlate, newPlate)
    ...
    if oldPlate == '' or newPlate == '' or oldPlate == newPlate then
        return true
    end
    ...
    if framework == 'qbcore' then
        if MySQL.single.await('SELECT plate FROM player_vehicles WHERE plate = ? LIMIT 1', { newPlate }) then
            return false, 'plate_already_in_framework_database'
        end
        affected = MySQL.update.await('UPDATE player_vehicles SET plate = ? WHERE citizenid = ? AND plate = ?', {
            newPlate, ownerIdentifier, oldPlate
        })
    elseif framework == 'esx' then
        if MySQL.single.await('SELECT plate FROM owned_vehicles WHERE plate = ? LIMIT 1', { newPlate }) then
            return false, 'plate_already_in_framework_database'
        end
        affected = MySQL.update.await('UPDATE owned_vehicles SET plate = ? WHERE owner = ? AND plate = ?', {
            newPlate, ownerIdentifier, oldPlate
        })
    else
    ...
    if affected == 1 and GetResourceState('ox_inventory') == 'started' then
        pcall(function()
...
function STVehicles.UpdateFrameworkOwner(oldOwnerIdentifier, newOwnerIdentifier, plate)
    ...
    if framework == 'qbcore' then
        affected = MySQL.update.await('UPDATE player_vehicles SET citizenid = ? WHERE citizenid = ? AND plate = ?', {
            newOwnerIdentifier, oldOwnerIdentifier, plate
        })
    elseif framework == 'esx' then
        affected = MySQL.update.await('UPDATE owned_vehicles SET owner = ? WHERE owner = ? AND plate = ?', {
            newOwnerIdentifier, oldOwnerIdentifier, plate
        })
    else
        return false, 'unsupported_framework'
...
function STVehicles.RegisterVehicle(data)
    ...
    if STVehicles.GetRegistration(data.vehicleIdentifier) then
        return false, 'vehicle_already_registered'
    end
    ...
    if isCustom then
        if not STValidation.IsCustomPlate(plate) then
            return false, 'invalid_custom_plate'
        end
        if plateExists(plate) then
            return false, 'plate_already_in_use'
        end
    else
        plate = generateUniquePlate()
        if not plate then
            return false, 'plate_generation_failed'
        end
    end

    local vin = data.vin or generateVIN()
    if not vin then
        return false, 'vin_generation_failed'
    end
    ...
    local insertId = MySQL.insert.await([[
        INSERT INTO st_vehicle_registrations
            (vehicle_identifier, vin, owner_identifier, owner_name, vehicle_model,
             vehicle_display_name, purchase_price, purchase_type, payment_method,
             financed, dealership, plate, plate_type, registered_at, expires_at, status)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'active')
    ]], {
        ...
        data.dealership,
        plate,
        isCustom and 'custom' or 'standard',
        now,
        expiresAt
    })
...
function STVehicles.SyncVehicleRecord(vehicleIdentifier)
    ...
    if not registration then
        return false, 'registration_not_found'
    end

    local insurance = MySQL.single.await([[
        SELECT policy_number, status, expires_at
        FROM st_vehicle_insurance
    ...
    MySQL.query.await([[
        INSERT INTO st_vehicle_records
            (vehicle_identifier, vin, plate, owner_identifier, owner_name,
             vehicle_model, vehicle_display_name, purchase_price, dealership,
             registration_status, registration_expires_at, insurance_policy_number,
        ...
        ON DUPLICATE KEY UPDATE
            ...
            vehicle_display_name = VALUES(vehicle_display_name),
            purchase_price = VALUES(purchase_price),
            dealership = VALUES(dealership),
            registration_status = VALUES(registration_status),
            registration_expires_at = VALUES(registration_expires_at),
            insurance_policy_number = VALUES(insurance_policy_number),
            insurance_status = VALUES(insurance_status),
            insurance_expires_at = VALUES(insurance_expires_at),
            updated_at = CURRENT_TIMESTAMP
    ]], {
        registration.vehicle_identifier,
        registration.vin,
        registration.plate,
        registration.owner_identifier,
        registration.owner_name or 'Unknown',
        registration.vehicle_model,
        registration.vehicle_display_name,
        registration.purchase_price or 0,
        registration.dealership,
        registration.status,
        registration.expires_at,
        insurance and insurance.policy_number or nil,
        insurance and insurance.status or nil,
        insurance and insurance.expires_at or nil
    })
...
function STVehicles.IsRegistered(plate)
    ...
    if not registration or registration.status ~= 'active' then
        return false
    end
