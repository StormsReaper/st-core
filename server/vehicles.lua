...
local function generateStandardPlate()
    return ('%s %s'):format(
        randomChars(3, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'),
        randomChars(3, '0123456789')
    )
end

local function plateExists(plate)
    local row = MySQL.single.await(
        'SELECT id FROM st_vehicle_registrations WHERE plate = ? LIMIT 1',
        { plate }
    )
...
function STVehicles.GetRegistrationByPlate(plate)
    ...
    return MySQL.single.await(
        'SELECT * FROM st_vehicle_registrations WHERE plate = ? LIMIT 1',
        { plate }
    )
end
...
function STVehicles.GetRegistration(vehicleIdentifier)
    ...
    return MySQL.single.await(
        'SELECT * FROM st_vehicle_registrations WHERE vehicle_identifier = ? LIMIT 1',
        { vehicleIdentifier }
    )
end
...
function STVehicles.RegisterVehicle(data)
    ...
    local existing = STVehicles.GetRegistration(data.vehicleIdentifier)
    if existing then
...
function STVehicles.RenewRegistration(vehicleIdentifier, durationDays)
    ...
    local affected = MySQL.update.await([[ 
        ...
        SET expires_at = FROM_UNIXTIME(?), status = 'active', updated_at = CURRENT_TIMESTAMP
        WHERE id = ?
...
function STVehicles.IsRegistered(plate)
    ...
    if registration.status ~= 'active' then
        return false
    end
