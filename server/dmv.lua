...
RegisterNetEvent('st-core:server:dmvData', function(targetSource)
    ...
    if not identifier then
        notify(source, result(false, 'Unable to identify your character.'))
        return
    end

    local pending = STPurchases.GetPending(source)
    local registrations = MySQL.query.await([[ 
        SELECT * FROM st_vehicle_registrations
    ...
    local policies = MySQL.query.await([[ 
        SELECT i.*, p.name AS plan_name, p.description AS plan_description,
               ...
               p.comprehensive_limit, p.deductible
    ...
    local plans = STInsurance.GetPlans()
...
RegisterNetEvent('st-core:server:registerVehicle', function(data)
    ...
    if not purchase or purchase.status ~= 'pending' then
        return notify(source, result(false, 'Vehicle is not awaiting registration.'))
    end
    if purchase.owner_identifier ~= identifier then
        return notify(source, result(false, 'You are not the registered purchaser.'))
    end

    local fee = tonumber(Config.Payment.RegistrationFee) or 0
    local paid, payError = STPayments.Charge(source, fee, 'DMV vehicle registration')
    ...
    local ok, registration = STVehicles.RegisterVehicle({
        ...
        vehicleIdentifier = purchase.vehicle_identifier,
        plate = data.customPlate
    })

    if not ok then
        STPayments.Add(source, fee, 'bank', 'DMV registration refund')
        return notify(source, result(false, registration))
    end

    STPurchases.MarkRegistered(purchase.vehicle_identifier)
    notify(source, result(true, 'Vehicle registered successfully.', registration))
...
RegisterNetEvent('st-core:server:renewRegistration', function(data)
    ...
    if not ok then
        STPayments.Add(source, fee, 'bank', 'DMV renewal refund')
...
RegisterNetEvent('st-core:server:buyInsurance', function(data)
    ...
    if not ok then
        STPayments.Add(source, plan.monthly_premium, 'bank', 'Insurance purchase refund')
...
RegisterNetEvent('st-core:server:renewInsurance', function(data)
    ...
    if not ok then
        STPayments.Add(source, 'bank', policy.premium, 'Insurance renewal refund')
        return notify(source, result(false, renewed))
    end
...
RegisterNetEvent('st-core:server:customPlate', function(data)
    ...
    if MySQL.single.await('SELECT id FROM st_vehicle_registrations WHERE plate = ? LIMIT 1', { plate }) then
        return notify(source, result(false, 'That plate is already in use.'))
    end
    ...
    local affected = MySQL.update.await([[ 
        ...
        SET plate = ?, plate_type = 'custom', updated_at = CURRENT_TIMESTAMP
        WHERE id = ?
    ]], { plate, registration.id })

    if affected ~= 1 then
        STPayments.Add(source, fee, 'bank', 'Custom plate refund')
        return notify(source, result(false, 'Unable to update plate.'))
    end
