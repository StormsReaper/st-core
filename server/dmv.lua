...
local function notify(source, payload)
    TriggerClientEvent('st-core:client:dmvResult', source, payload)
end
...
RegisterNetEvent('st-core:server:dmvData', function(targetSource)
    ...
    TriggerClientEvent('st-core:client:dmvData', source, {
        ...
        fees = {
            customPlate = Config.Plate.CustomPlateFee,
            registration = Config.Payment.RegistrationFee,
            transfer = Config.Registration.TransferFee,
        },
    })
end)
...
RegisterNetEvent('st-core:server:lookupVehicleByPlate', function(data)
    ...
    if plate == '' then return notify(source, result(false, 'Enter the vehicle plate number.')) end

    local lookup, err = STVehicles.LookupOwnedVehicleForDMV(identifier, STPayments.GetName(source), plate)
    if not lookup then
        return notify(source, result(false, err == 'vehicle_not_found_or_not_owned'
            and 'No vehicle with that plate is registered to your character.'
...
RegisterNetEvent('st-core:server:registerVehicle', function(data)
    ...
    local enteredPlate = STValidation.NormalizePlate(data.lookupPlate or data.plateNumber or '')
    if enteredPlate == '' then return notify(source, result(false, 'Enter the vehicle plate number first.')) end
    ...
    local ok, registration = STVehicles.RegisterVehicle({
        ...
        ownerName = STPayments.GetName(source),
    ...
    if not ok then
        STPayments.Add(source, total, paymentAccountOrError or 'bank', 'DMV registration refund')
        return notify(source, result(false, registration))
    end

    local synced = syncFrameworkPlate(identifier, enteredPlate, registration.plate)
    if not synced then
        MySQL.query.await('DELETE FROM st_vehicle_registrations WHERE id = ?', { registration.id })
        STPayments.Add(source, total, paymentAccountOrError or 'bank', 'DMV registration refund')
        return notify(source, result(false, 'The framework vehicle plate could not be updated; registration was rolled back.'))
    end
    ...
    notify(source, result(true,
        ('Vehicle registered successfully. Plate: %s. Total paid: $%s'):format(registration.plate, total),
        STVehicles.GetRegistration(vehicle.vehicleIdentifier)))
...
RegisterNetEvent('st-core:server:renewInsurance', function(data)
    ...
    if not ok then
        STPayments.Add(source, policy.premium, paymentAccountOrError or 'bank', 'Insurance renewal refund')
        return notify(source, result(false, renewed))
    end

    renewed.plate = policy.plate
    renewed.insured_name = policy.insured_name or STPayments.GetName(source)
...
RegisterNetEvent('st-core:server:customPlate', function(data)
    ...
    if not synced then
        STPayments.Add(source, fee, paymentAccountOrError or 'bank', 'Custom plate refund')
        return notify(source, result(false, syncError or 'Unable to update vehicle plate.'))
    end

    if MySQL.update.await("UPDATE st_vehicle_registrations SET plate = ?, plate_type = 'custom', updated_at = CURRENT_TIMESTAMP WHERE id = ?", { plate, registration.id }) ~= 1 then
    ...
    STVehicles.SyncVehicleRecord(registration.vehicle_identifier)
