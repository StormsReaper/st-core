...
local function generatePolicyNumber()
    for _ = 1, (Config.Insurance.PolicyGenerationAttempts or 50) do
        local policy = ('%s-%06d'):format(Config.Insurance.PolicyPrefix or 'STI', math.random(0, 999999))
        local exists = MySQL.single.await('SELECT id FROM st_vehicle_insurance WHERE policy_number = ? LIMIT 1', { policy })
        if not exists then return policy end
    end
    return nil
end
...
function STInsurance.GetVehicleLegalStatus(vehicleIdentifier)
    ...
    local policy = STInsurance.GetPolicy(vehicleIdentifier)
    local registrationActive = registration ~= nil and registration.status == 'active' and tonumber(registration.expires_at) >= os.time()
